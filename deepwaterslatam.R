...
deepwaters=raster("gebco_2023_n13.7549_s-57.7002_w-85.4297_e-30.5859.tif")  # read GeoTIFF file
deepwaters
plot(deepwaters)

hist(as.matrix(deepwaters), breaks=800)
max(as.matrix(deepwaters))  # 6787m (Aconcagua, 6961m)
min(as.matrix(deepwaters))  # -8997m

# RESAMPLE raster to 1080
DIMY=1080
DIMX=830  # .h264 needs even dimensions
...
max(DEM)  # 5943.97m (Aconcagua, 6961m)
min(DEM)  # -7738.545

DEM=DEM-min(DEM)
writeTIFF(DEM/max(DEM), "deepwaters2.tif",
...
outline[outline!=0]=1
writePNG(outline, "mapoutline2.png")
...
NFRAMES=2556  # 106.5s audio track at 24fps
...
for (frame in 0:(NFRAMES-1)) {
    ...
    map[800:(800+63-1),(590-(LONG-1)/2*44):(590-(LONG-1)/2*44+251-1),1][meters]=0
    map[800:(800+63-1),(590-(LONG-1)/2*44):(590-(LONG-1)/2*44+251-1),2][meters]=0
    map[800:(800+63-1),(590-(LONG-1)/2*44):(590-(LONG-1)/2*44+251-1),3][meters]=0
...
Offset=2556  # previously generated frames
NFRAMES=1204  # 50.186s audio track at 30fps
...
# Building MP4:
# ffmpeg -framerate 24 -i img%4d.png -i tronrecognizer.wav
#       -c:v libx264 -crf 18 -pix_fmt yuv420p deepwaterslatam.mp4