...
deepwaters=raster("gebco_2023_n67.2363_s-8.5254_w-130.7812_e-50.9766.tif")  # read GeoTIFF file
deepwaters
plot(deepwaters)
...
centredextent=(extent(deepwaters)[1]+extent(deepwaters)[2])/2
CRS=paste0("+proj=lcc +lon_0=", centredextent,
           " +units=km")  # by default crs="+proj=lcc +ellps=WGS84 +lat_1=33 +lat_2=45 +lon_0=0"
deepwatersrp=projectRaster(deepwaters, crs=CRS)
deepwatersrp
plot(deepwatersrp)
abline(v=0)  # Greenwich meridian
abline(v=c(-3000, 3000), h=c(700, 6000))

# CROP raster to area of interest (save Canary Islands and The Netherlands)
cropdef=as(extent(-3000, 3000, 700, 6000), 'SpatialPolygons')  # kms
...
DIMY=round(1920*((6000-700)/(3000--3000)))
DIMX=1920
...
# Outline correction for Great Lakes:

# Superior, Michigan, Huron, Erie: ~179m above sea level (0m)
# Calculate solid map contour
solid=DEM
solid[solid<179]=0  # set <0 areas to 0 (water)
solid[solid>=179]=1  # set >=0 areas to 1 (land)
writePNG(solid, "mapsolid_179m.png")
...
outline[2:(DIMY-1), 2:(DIMX-1)]=
    ...
    abs(solid[2:(DIMY-1), 1:(DIMX-2)] -
        solid[2:(DIMY-1), 2:(DIMX-1)])
outline[outline!=0]=1
writePNG(outline, "mapoutline_179m.png")

# Ontario: 74m above sea level (0m)
# Calculate solid map contour
solid=DEM
solid[solid<74]=0  # set <0 areas to 0 (water)
solid[solid>=74]=1  # set >=0 areas to 1 (land)
writePNG(solid, "mapsolid_74m.png")
...
outline[2:(DIMY-1), 2:(DIMX-1)]=
    ...
    abs(solid[2:(DIMY-1), 1:(DIMX-2)] -
        solid[2:(DIMY-1), 2:(DIMX-1)])
outline[outline!=0]=1
writePNG(outline, "mapoutline_74m.png")

# Photoshop layers...
outline=readPNG("mapoutline_greatlakes.png")
...
NFRAMES=2172  # 93.8s audio track at 24fps
...
for (frame in 0:(NFRAMES-1)) {
    ...
    map[450:(450+63-1),(946-(LONG-1)/2*44):(946-(LONG-1)/2*44+251-1),1][meters]=0
    map[450:(450+63-1),(946-(LONG-1)/2*44):(946-(LONG-1)/2*44+251-1),2][meters]=0
    map[450:(450+63-1),(946-(LONG-1)/2*44):(946-(LONG-1)/2*44+251-1),3][meters]=0
...
Offset=2172  # previously generated frames
NFRAMES=1246  # 52.4s audio track at 24fps
...
# Building MP4:
# ffmpeg -framerate 30 -i img%4d.png -i expanseepicorchestral.wav
#        -c:v libx264 -crf 18 -pix_fmt yuv420p deepwatersusa.mp4
