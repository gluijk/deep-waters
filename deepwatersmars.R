...
# READ raster
deepwaters=rast("Mars_MGS_MOLA_DEM_mosaic_global_463m.tif")  # read GeoTIFF file
deepwaters
...
deepwaters=crop(x=deepwaters, y=cropdef)
deepwaters
plot(deepwaters)
...
DIMY=1080
DIMX=1920
deepwatersrs=rast(nrows=DIMY, ncols=DIMX, extent=ext(deepwaters))
deepwatersrs=resample(x=deepwaters, y=deepwatersrs, method='bilinear', threads=TRUE)
deepwaters
plot(deepwatersrs)
RESOLUTION=res(deepwatersrs)[1]


# Convert to matrix and save as TIFF
DEM=as.array(deepwatersrs)
DEM=matrix(DEM, nrow=nrow(deepwatersrs))
...
hill=hillshade(DEM, dx=RESOLUTION*0.4, dlight=c(0, 2, 3))
hillfill=hillshade(DEM, dx=RESOLUTION*0.4, dlight=c(0, 3, 2))
...
hill=readTIFF("hillshadesharpen.tif")  # sharpen + labels in Photoshop
...
DEM=DEM-min(DEM)  # normalize to 0m
MAXHEIGHT=max(DEM)
MINHEIGHT=min(DEM)
COLUP=1.8  # 2
COLDOWN=0.56  # 0.5
...
NFRAMES=2205  # 91.895s audio track at 24fps
...
Offset=2205  # previously generated frames
NFRAMES=675  # 28.105s audio track at 24fps
...
# Building MP4:
# ffmpeg -framerate 24 -i img%4d.png -i bobafett.wav
#        -c:v libx264 -crf 18 -pix_fmt yuv420p deepwatersmars.mp4
