...
deepwaters=raster("gebco_2023_n38.2324_s-38.584_w-22.5_e57.1289.tif")  # read GeoTIFF file
deepwaters
plot(deepwaters)
...
deepwatersrs=raster(nrow=DIMY, ncol=DIMX,
                    ext=extent(deepwaters), crs=crs(deepwaters))
deepwatersrs=resample(deepwaters, deepwatersrs,
...
# Outline correction for Lake Victoria: 1208mm above sea level (0m)
# Calculate solid map contour
solid=DEM
solid[solid < 1208]=0  # set <0 areas to 0 (water)
solid[solid >= 1208]=1  # set >=0 areas to 1 (land)
writePNG(solid, "mapsolid_1208m.png")
...
outline[2:(DIMY-1), 2:(DIMX-1)]=
    ...
    abs(solid[2:(DIMY-1), 1:(DIMX-2)] -
        solid[2:(DIMY-1), 2:(DIMX-1)])
outline[outline!=0]=1
writePNG(outline, "mapoutline_1208m.png")


# Outline correction for Lake Tanganyica: 773mm above sea level (0m)
# Calculate solid map contour
solid=DEM
solid[solid < 773]=0  # set <0 areas to 0 (water)
solid[solid >= 773]=1  # set >=0 areas to 1 (land)
writePNG(solid, "mapsolid_773m.png")
...
outline[2:(DIMY-1), 2:(DIMX-1)]=
    ...
    abs(solid[2:(DIMY-1), 1:(DIMX-2)] -
            solid[2:(DIMY-1), 2:(DIMX-1)])
outline[outline!=0]=1
writePNG(outline, "mapoutline_773m.png")


# Outline correction for Lake Tanganyica: 460mm above sea level (0m)
# Calculate solid map contour
solid=DEM
solid[solid < 460]=0  # set <0 areas to 0 (water)
solid[solid >= 460]=1  # set >=0 areas to 1 (land)
writePNG(solid, "mapsolid_460m.png")
...
outline[2:(DIMY-1), 2:(DIMX-1)]=
    ...
    abs(solid[2:(DIMY-1), 1:(DIMX-2)] -
            solid[2:(DIMY-1), 2:(DIMX-1)])
outline[outline!=0]=1
writePNG(outline, "mapoutline_460m.png")

# Photoshop layers...
outline=readPNG("mapoutline_africalakes.png")


# Calculate grayscale hillshade
MIX=0.85  # two light sources are mixed to fill dark areas a bit
hill=hillshade(DEM, dx=700, dlight=c(0, 2, 3))
hillfill=hillshade(DEM, dx=500, dlight=c(0, 3, 2))
...
NFRAMES=2587  # 107.8s audio track at 24fps
...
for (frame in 0:(NFRAMES-1)) {
    ...
    map[290:(290+63-1),(510-(LONG-1)/2*44):(510-(LONG-1)/2*44+251-1),1][meters]=0
    map[290:(290+63-1),(510-(LONG-1)/2*44):(510-(LONG-1)/2*44+251-1),2][meters]=0
    map[290:(290+63-1),(510-(LONG-1)/2*44):(510-(LONG-1)/2*44+251-1),3][meters]=0
...
Offset=2587  # previously generated frames
NFRAMES=1136  # 47.3s audio track at 24fps
...
for (frame in 0:(NFRAMES-1)) {
    ...
    label=t(label[,ncol(label):1])
    # Stroke=3
    label[1:61,2:(250+44)]=label[1:61,2:(250+44)]+label[2:62,2:(250+44)]
    label[3:63,2:(250+44)]=label[3:63,2:(250+44)]+label[2:62,2:(250+44)]
    label[2:62,1:(249+44)]=label[2:62,1:(249+44)]+label[2:62,2:(250+44)]
    label[2:62,3:(251+44)]=label[2:62,3:(251+44)]+label[2:62,2:(250+44)]
    meters=which(label!=0)
    map[290:(290+63-1),(510-(LONG-1)/2*44):(510-(LONG-1)/2*44+251-1+44),1][meters]=0
    map[290:(290+63-1),(510-(LONG-1)/2*44):(510-(LONG-1)/2*44+251-1+44),2][meters]=0
    map[290:(290+63-1),(510-(LONG-1)/2*44):(510-(LONG-1)/2*44+251-1+44),3][meters]=0
...
# Building MP4:
# ffmpeg -framerate 24 -i img%4d.png -i tronendofline.wav
#        -c:v libx264 -crf 18 -pix_fmt yuv420p deepwatersafrica.mp4
