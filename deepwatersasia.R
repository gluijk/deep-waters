...
deepwaters=raster("gebco_2023_n58.0957_s0.4395_w38.6719_e149.7656.tif")  # read GeoTIFF file
deepwaters
plot(deepwaters)
...
# Outline correction for Caspian Sea: 27m below sea level (0m)
# Calculate solid map contour
solid=DEM
solid[solid >= -27]=1  # set >=0 areas to 1 (land)
solid[solid < -27]=0  # set <0 areas to 0 (water)
writePNG(solid, "mapsolid_-27m.png")
...
outline[2:(DIMY-1), 2:(DIMX-1)]=
    ...
    abs(solid[2:(DIMY-1), 1:(DIMX-2)] -
        solid[2:(DIMY-1), 2:(DIMX-1)])
outline[outline!=0]=1
writePNG(outline, "mapoutline_-27m.png")

# Photoshop layers...
outline=readPNG("mapoutline_caspiansea.png")


# Calculate grayscale hillshade
MIX=0.85  # two light sources are mixed to fill dark areas a bit
hill=hillshade(DEM, dx=500, dlight=c(0, 2, 3))
...
NFRAMES=2388  # 99.5s audio track at 24fps
...
for (frame in 0:(NFRAMES-1)) {
    ...
    map[100:(100+63-1),(500-(LONG-1)/2*44):(500-(LONG-1)/2*44+251-1),1][meters]=0
    map[100:(100+63-1),(500-(LONG-1)/2*44):(500-(LONG-1)/2*44+251-1),2][meters]=0
    map[100:(100+63-1),(500-(LONG-1)/2*44):(500-(LONG-1)/2*44+251-1),3][meters]=0
...
Offset=2388  # previously generated frames
NFRAMES=386  # 16.1s audio track at 24fps
...
for (frame in 0:(NFRAMES-1)) {
    ...
    meters=which(label!=0)
    map[100:(100+63-1),(500-(LONG-1)/2*44):(500-(LONG-1)/2*44+251-1+44),1][meters]=0
    map[100:(100+63-1),(500-(LONG-1)/2*44):(500-(LONG-1)/2*44+251-1+44),2][meters]=0
    map[100:(100+63-1),(500-(LONG-1)/2*44):(500-(LONG-1)/2*44+251-1+44),3][meters]=0

    writePNG(map, paste0("CHECK_", ifelse(frame+Offset<10, "000",
...
# Building MP4:
# ffmpeg -framerate 24 -i img%4d.png -i interstellarnotimeforcaution.wav
#        -c:v libx264 -crf 18 -pix_fmt yuv420p deepwatersasia.mp4
